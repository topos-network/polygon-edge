...
type networkingServer interface {
	...
	UpdatePendingFrostConnCount(delta int64, direction network.Direction)
...
func (f *FrostService) GetNotifyBundle() *network.NotifyBundle {
	return &network.NotifyBundle{
		ConnectedF: func(net network.Network, conn network.Conn) {
			...
			if f.hasPendingStatus(peerID) {
				// handshake has already started
				return
			}

			if !f.baseServer.HasFreeFrostConnectionSlot(conn.Stat().Direction) {
			...
			f.addPendingStatus(peerID, conn.Stat().Direction)
...
func (i *FrostService) handleConnected(peerID peer.ID, direction network.Direction) error {
	...
	fmt.Println(">>>>>>>>>>>>> Here we should communicate and check frost info of the peer: ", peerID)
